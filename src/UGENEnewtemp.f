c     Program to get free energies at 37 degrees celsius and enthalpies
c     from input files and calculate the free energies at a different
c     temperature.  For DNA, [Na+] and [Mg++] can also be varied.
c     -----------------------------------------------------------------

      program newtemp
      
      character cur_executable_path*4096, acid*3, naconc*10, mgconc*10, olipoly*7
...
c     If nucleic acid type not entered in command line, ask user
     ...
     .     (index(acid,'RNA').eq.0.and.index(acid,'rna').eq.0)) then
         write (6,*) 'DNA or RNA files? (default = RNA)'
         read (5,900) acid
...
c     If the temperature not entered in the command line, ask user
      if (index(chtemp,'   ').gt.0) then
         write (6,*) 'Enter new temperature (int in deg celsius)'
         read (5,900) chtemp
         if (index(chtemp,'   ').gt.0) then
            write (6,*) 'Enter new temperature (int in deg celsius)'
            read (5,900) chtemp
            if (index(chtemp,'   ').gt.0) then
               ...
               write (6,*) 'Rerun with correct temperature.'
               call exit(1)
            endif
         endif
      endif
...
c     If request Na+ or Mg++ concentration if not in command line.
      if (naconc.eq.'          ') then
...
c     Test for input that specifies mM.
 ...
 810  format(a3,' free energies. [Na+] = ',f6.4,' M [Mg++] = ',
     .     f6.4,' M')
      if (olipoly.eq.'       '.or.(olipoly.ne.'polymer'.and.
     .     olipoly.ne.'POLYMER')) then
         olipoly = 'oligo  '
         salt_corr1 = ion(acid,olipoly,'      ',float(temper),rnaconc,
     .        rmgconc)
         salt_corr2 = salt_corr1
      else
         salt_corr1 = ion(acid,olipoly,'affine',float(temper),rnaconc,
     .        rmgconc)
         salt_corr2 = ion(acid,olipoly,'linear',float(temper),rnaconc,
     .        rmgconc)
      endif
      
c     Determine what the filename suffix should be based on the nucleic
c     acid
...
c     Asymmetric interior 1x2 and then 2x3
      call asint1x2(cur_executable_path)
c     call asint2x3  Zuker disables this for now
      
c     Dangles
      call dangle(cur_executable_path)
      
c     Loop
      call loop(cur_executable_path)
      
c     Miscellaneous Loops
      call miscloop(cur_executable_path)
      
c     Symmetric interior loops of sizes 2, 4, and 6
      call sint2(cur_executable_path)
      call sint4(cur_executable_path)
c     call sint6  Zuker disables this for now
      
c     Stacking energies
      call stack(cur_executable_path)
      
c     Tetra-loops
      call tloop(cur_executable_path)
      
c     Triloops
      call triloop(cur_executable_path)
...
c     Reads headers and data from asint1x2.dg(d)
      itemp=0
      do while(index(a1header(itemp),'(U)').eq.0.and.
     .     index(a1header(itemp),'(T)').eq.0)
         itemp=itemp+1
...
c     Reads real, numerical, free energy data and stores it in ra1garray
         do jtemp=1,4
...
c     Calculates free energy at specified temperature
     ...
     .              real(temper))*ra1harray(itemp,jtemp,iptemp)) + 
     .              ((273.15 + real(temper))*
     .              ra1garray(itemp,jtemp,iptemp))) + salt_corr1
     .              + 1.5*salt_corr2
            enddo
         enddo
      enddo
...
c     Asymmetric interior 2 x 3
      ...
      character as2gfile*12,as2hfile*12,path*80,a2header(50)*70,
     .     a2tables(384,0:30)*120,rowdown*20,a2outfile*12,row*20
      integer itemp,jtemp,iptemp
...
c     Read in data from asint2x3.dg(d) -- free energies
      itemp=0
      do while(index(a2header(itemp),'(U)').eq.0.and.
     .     index(a2header(itemp),'(T)').eq.0)
         itemp=itemp+1
...
c     Calculates free energy at specified temperature
     ...
     .              real(temper))*ra2harray(itemp,jtemp,iptemp)) + 
     .              ((273.15 + real(temper))*ra2garray(itemp,jtemp,
     .              iptemp))) + salt_corr1 + 2.5*salt_corr2
            enddo
         enddo
      enddo
...
c     Makes sure value read in is number, then calculates output
 ...
 95   write (6,*) 'Error reading enthalpies; itemp=',itemp,
     .     ' jtemp=',jtemp
      write (6,*) 'enthalpies(itemp,jtemp)=',
     .     enthalpies(itemp,jtemp)
      call exit(1)
      end
...
c     Check for overflow
      if (num.le.-100.0.or.num.ge.1000.0) then
         write (6,*) 'ERROR in num2char subroutine. Value = ',num
      endif   
      return
      end
c--------------------------------------------------------------------
...
c     Sets numrow to '......' so can enter do while loop
 ...
 99   max=i
c     Calculates the free energy at the specified temp and stores it in
c     outchar
      do i=1,max
         do j=1,3
...
c     Reads in data from sint2.dg(d)
      i=0
      do while(index(mainheader(i),'(U)').eq.0.and.
     .     index(mainheader(i),'(T)').eq.0)
         i=i+1
...
c     Calculates free energy at specified temp
     ...
     .              (273.15 + real(temper))*fearray(i,j,k))/310.15 + 
     .              salt_corr1 + salt_corr2
            enddo
         enddo
      enddo
...
c     Stores the mainheader in mainheader character array
      itemp=0
      do while(index(mainheader(itemp),'(UU)').eq.0.
     .     and.index(mainheader(itemp),'(TT)').eq.0)
         itemp=itemp+1
...
c     Calculates output data
      do itemp=1,36
         do jtemp=1,16
            do ktemp=1,16
               output(itemp,jtemp,ktemp) = (1./310.15)*(((37.0 - 
     .              real(temper))*harray(itemp,jtemp,ktemp)) +
     .              ((273.15+real(temper))*garray(itemp,jtemp,ktemp))) +
     .              salt_corr1 + 2.0*salt_corr2
            enddo
         enddo
      enddo
...
c     Writes the mainheader to the output file
      itemp=0
      do while(index(mainheader(itemp),'(UU)').eq.0.and.
     .     index(mainheader(itemp),'(TT)').eq.0)
         itemp=itemp+1
...
c     Reads in headers and data from sint6.dg(d)
      itemp=0
      do while(index(header(itemp),'(U)').eq.0.and.
     .     index(header(itemp),'(T)').eq.0)
         itemp=itemp+1
...
c     write (6,*) 'itemp=',itemp,' jtemp=',jtemp,
c     .              ' ktemp=',ktemp
c     call exit(1)
c     endif
c     enddo
...
c     Calculates free energies at specified temperature
         do itemp=1,4
            do jtemp=1,4
               do ktemp=1,16
                  if (index(gcharray(itemp,jtemp,ktemp),' . ').eq.0) 
     .                 then
